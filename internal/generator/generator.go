...
type Generator struct {
	...
	attrPolicy AttributeSupportPolicy
}

func New(s *schema.Schema) *Generator {
	return NewWithAttributeSupportPolicy(s, DefaultAttributeSupportPolicy())
}

func NewWithAttributeSupportPolicy(s *schema.Schema, policy AttributeSupportPolicy) *Generator {
	return &Generator{
		schema:     s,
		fakes:      NewFakeRegistry(),
		validator:  validation.NewWithAttributeFilter(s, policy.Allows),
		attrPolicy: policy,
	}
}
...
func (g *Generator) buildRecord(ctx context.Context, cfg GeneratorConfig, typ EntryType, index int, rel Relationships, rng *rand.Rand) (ldif.Record, error) {
	...
	for _, name := range mergeAttributes(resolved.Must) {
		if err := g.addGeneratedAttribute(ctx, &rec, cfg, name, ec, true); err != nil {
			return rec, err
		}
	}
	for _, name := range chooseMay(resolved.May, cfg, rng) {
		if err := g.addGeneratedAttribute(ctx, &rec, cfg, name, ec, false); err != nil {
			return rec, err
		}
	}
	g.addRelationshipAttributes(&rec, resolved, ec)
	for _, extra := range rel.ExtraAttributes[ec.DN] {
		attr, ok := g.schema.ResolveAttributeType(extra.Name)
		if ok && recordHasAttribute(rec, attr) {
			continue
		}
		if ok && !g.canGenerateAttribute(attr) {
			continue
		}
		values := extra.Values
...
func (g *Generator) addGeneratedAttribute(ctx context.Context, rec *ldif.Record, cfg GeneratorConfig, name string, ec EntryContext, required bool) error {
	if schema.NormalizeName(name) == "objectclass" {
		return nil
	}
	if isReservedGeneratedAttribute(name) {
		if required {
			return fmt.Errorf("required attribute %q is reserved and cannot be generated", name)
		}
		return nil
	}
	attr, ok := g.schema.ResolveAttributeType(name)
	if !ok {
		if cfg.StrictMode {
			return fmt.Errorf("attribute %q is not defined in schema", name)
		}
		attr = schema.AttributeType{Names: []string{name}}
	}
	if attr.NoUserMod {
		if required {
			return fmt.Errorf("required attribute %q is NO-USER-MODIFICATION and cannot be generated by client", name)
		}
		return nil
	}
	if !g.attrPolicy.Allows(attr) {
		if required {
			return fmt.Errorf("required attribute %q is disabled by generator attribute policy", name)
		}
		return nil
	}
	...
	if len(values) == 0 {
		if required {
			return fmt.Errorf("required attribute %q generated no values", name)
		}
		return nil
	}
...
func (g *Generator) canGenerateAttribute(attr schema.AttributeType) bool {
	return !attr.NoUserMod && g.attrPolicy.Allows(attr)
}
