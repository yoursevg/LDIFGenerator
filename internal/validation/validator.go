...
type Validator struct {
	...
	attributeAllowed func(schema.AttributeType) bool
}

func New(s *schema.Schema) *Validator {
	return NewWithAttributeFilter(s, nil)
}

func NewWithAttributeFilter(s *schema.Schema, attributeAllowed func(schema.AttributeType) bool) *Validator {
	if attributeAllowed == nil {
		attributeAllowed = func(schema.AttributeType) bool { return true }
	}
	return &Validator{schema: s, attributeAllowed: attributeAllowed}
}
...
func (v *Validator) ValidateRecord(record ldif.Record, objectClasses []string, strict bool) error {
	...
	for _, attr := range record.Attributes {
		...
		if strict {
			schemaAttr, ok := v.schema.ResolveAttributeType(key)
			...
			if ok && !v.attributeAllowed(schemaAttr) {
				return fmt.Errorf("attribute %q is disabled by generator attribute policy", attr.Name)
			}
			if !allowed[key] {
...
func (v *Validator) isNoUserModification(name string) bool {
	attr, ok := v.schema.ResolveAttributeType(name)
...
func (v *Validator) attributeKeys(name string) []string {
	...
	if attr, ok := v.schema.ResolveAttributeType(name); ok {
