import { Play, Save, Square, Upload } from "lucide-react";
import type { ReactNode } from "react";
import { useEffect, useMemo, useState } from "react";
import { cancelGeneration, defaultConfig, generate, loadConfig, loadSchema, progress, saveConfig } from "./api";
...
export default function App() {
  ...
  const [configPath, setConfigPath] = useState("examples/config.json");
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(async () => {
      try {
        ...
        if (!p.running) {
          setRunning(false);
          setStatus(p.report ? "Генерация завершена" : "Генерация остановлена");
        }
      } catch (err) {
  ...
  async function onSaveConfig() {
    if (!config) return;
    try {
      await saveConfig(configPath, normalizeConfig(config));
      ...
      <section className="workspace">
        ...
        <div className="panel config-panel">
          ...
          <div className="form-grid single">
            <Field label="Config path">
              <div className="inline">
                ...
                <button onClick={onLoadConfig} title="Load config"><Upload size={17} /></button>
                <button onClick={onSaveConfig} title="Save config"><Save size={17} /></button>
              </div>
            </Field>
            <Field label="Base DN"><input value={config.baseDN} onChange={(e) => updateConfig({ baseDN: e.target.value })} /></Field>
            <Field label="Output LDIF"><input value={config.outputPath} onChange={(e) => updateConfig({ outputPath: e.target.value })} /></Field>
            ...
            <Field label="Seed"><input type="number" value={config.seed} onChange={(e) => updateConfig({ seed: Number(e.target.value) })} /></Field>
            <Field label="Batch size"><input type="number" value={config.batchSize} onChange={(e) => updateConfig({ batchSize: Number(e.target.value) })} /></Field>
          </div>
          ...
          <div className="form-grid">
            ...
            <Field label="Групп со всеми users"><input type="number" value={config.relationships.allUsersGroupCount} onChange={(e) => updateRel("allUsersGroupCount", Number(e.target.value))} /></Field>
            ...
            <Field label="Max members/group"><input type="number" value={config.relationships.maxMembersPerGroup} onChange={(e) => updateRel("maxMembersPerGroup", Number(e.target.value))} /></Field>
          </div>
        </div>
      </section>
    </main>
